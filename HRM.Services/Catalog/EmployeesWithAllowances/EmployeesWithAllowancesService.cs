...
namespace HRM.Services.Catalog.EmployeesWithAllowances
{
    public class EmployeesWithAllowancesService : IEmployeesWithAllowancesService
    {
        ...
        public EmployeesWithAllowancesService(HRMDbContext context, IWorkHourService workHourService)
        {
            _context = context;
            _workHourService = workHourService;
        }

        public async Task<List<HRM.Entity.RelationShips.EmployeesWithAllowances>> check(EwaAutoCheckRequest request)
        {
        ...
        public async Task<int> auto(EwaAutoCheckRequest request)
        {
            ...
            foreach (var item in query)
            {
                ...
                if (kt == 0)
                {
                    var ewa = new HRM.Entity.RelationShips.EmployeesWithAllowances()
                    {
